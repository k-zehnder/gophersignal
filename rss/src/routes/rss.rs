...
/// Computes the publication date for an article, using `created_at`
/// and applying the article rank as an offset (by adding a small time duration).
fn compute_pub_date(article: &Article) -> String {
    // Apply a small time offset based on the article's rank
    let rank_offset = Duration::minutes(article.article_rank as i64);

    let pub_date = DateTime::parse_from_rfc3339(&article.created_at)
        .unwrap_or_else(|_| Utc::now().into())
        .checked_add_signed(rank_offset)
        .unwrap()
        .to_rfc2822();
...
pub async fn generate_rss_feed<T: ArticlesClient + Clone>(
    ...
    // Sort articles by article rank (lowest rank first)
    articles.sort_by(|a, b| a.article_rank.cmp(&b.article_rank));
