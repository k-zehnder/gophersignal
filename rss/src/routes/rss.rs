...
fn build_item(article: &Article) -> rss::Item {
    let (guid_value, is_permalink) = if matches!(
        article.comment_link.as_deref(),
        Some(cl) if !cl.trim().is_empty() && cl.starts_with("https://news.ycombinator.com/item?id=")
    ) {
        // Valid HN discussion URL means permalink=true
        (article.comment_link.clone().unwrap(), true)
    } else {
        // Fallback to SHA-1 of title
        let hash = format!(
            "{:x}",
